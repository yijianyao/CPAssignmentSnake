...
public class Login extends JFrame implements Runnable{
		...
		loginButton.addActionListener(new ActionListener(){		
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(validation(nameTextbox.getText(), passwordTextbox.getPassword(),"user")){
					...
					//buffer.append(nameTextbox.getText());
	...
	public void run() {
		...
		//System.out.println("Buffer add times");
		// concurrent add user
		
		while(!this.nameTextbox.getText().equalsIgnoreCase("") ) {
			System.out.println("Buffer add times");
			for(int i = 0; i < 5 ;i++) {
				buffer.append(nameTextbox.getText());
				System.out.println("Buffer add " + i + " times");
			}
